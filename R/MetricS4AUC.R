...
.get_available_auc_roc_metrics <- function() return(c("auc", "auc_roc"))



#####is_available#####
setMethod("is_available", signature(object="familiarMetricAUCROC"),
          function(object, ...){
...
setMethod("compute_metric_score", signature(metric="familiarMetricAUCROC"),
          function(metric, data, ...){
            ...
            outcome_classes <- get_outcome_class_levels(data, outcome_type=metric@outcome_type)
            ...
            # Generate empty auc vector AUC of the ROC is calculated according
            # to Hand, D.J, and Till, R.J. A simple generalisation of the area
            ...
            for(ii in seq_len(n_class_combinations)){
              ...
              # Get number of positive and negative class entries (n0 and n1 in
              # Hand et al.).
              n_positive <- length(class_probability_positive)
              n_negative <- length(class_probability_negative)
              
              # Calculate AUC only when positive and negative classes are
              # present.
              if(n_positive > 0 & n_negative >0){
                # Determine probability ranks
                sample_rank <- data.table::frank(x=c(class_probability_positive, class_probability_negative),
                                                 ties.method="average")
                
                # Calculate AUC
                auc_score[ii] <- (sum(sample_rank[seq_len(n_positive)]) - n_positive * (n_positive + 1) / 2) / (n_positive * n_negative)
            ...
            # Calculate mean AUC (eq. 7 from Hand et al.). This has no effect
            # for binomial AUC.
