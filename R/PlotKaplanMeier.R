...
#'  `data_set`, colouring of the strata in each individual plot by `risk_group`.
#'
...
setMethod("plot_kaplan_meier", signature(object="familiarCollection"),
          function(object,
            ...
            if(is.list(x)){
              ...
              if(length(x) > 1) ..error_reached_unreachable_code("plot_kaplan_meier: list of data elements contains unmerged elements.")
            ...
            split_var_list <- plotting.check_data_handling(x=x@data,
                                                           ...
                                                           linetype_by=linetype_by,
                                                           facet_by=facet_by,
                                                           available=c("fs_method", "learner", "data_set", "risk_group", "stratification_method"))
            ...
            for(current_split in data_split){
              ...
              if(!is.null(current_split)){
                x_split <- methods::new("familiarDataElementRiskStratification",
                                        x,
...
.plot_kaplan_meier <- function(x,
  ...
  plot_layout_table <- plotting.get_plot_layout_table(x=x@data,
                                                      facet_by=facet_by,
                                                      facet_wrap_cols=facet_wrap_cols)
  ...
  for(current_split in data_split){
    ...
    strata <- .compute_risk_stratification_curves(x=x_split,
                                                  time_range=x_range)
    ...
    # Kaplan-Meier plots
    p_kaplan_meier <- .create_km_subplot(x=strata,
                                         h=test_data,
                                         color_by=color_by,
                                         linetype_by=linetype_by,
                                         facet_by=facet_by,
    ...
    if(show_survival_table & gtable::is.gtable(g_kaplan_meier)){
      # Survival tables
      p_survival_table <- .create_survival_table_subplot(x=strata,
                                                         color_by=color_by,
                                                         linetype_by=linetype_by,
                                                         ggtheme=ggtheme,
                                                         discrete_palette=discrete_palette,
...
.create_km_subplot <- function(x,
  ...
  if(show_logrank & !is_empty(h)){
    # Parse p-value
    p_value_label <- paste0("p: ", as.character(signif(h[risk_group_1=="all"]$p_value, 2)))
...
.create_survival_table_subplot <- function(x,
                                           color_by,
                                           linetype_by,
                                           ggtheme,
                                           discrete_palette,
  ...
  if(is.null(c(color_by, linetype_by))){
    ...
    # Convert to factors
    for(ii in unique_vars) guide_table[[ii]] <- factor(guide_table[[ii]], levels=levels(x@data[[ii]]))
    ...
    if(any(guide_table$missing_entry)){
      ...
      proto_table <- merge(x=survival_table,
      ...
      survival_table <- data.table::rbindlist(c(list(survival_table), new_survival_list),
                                              use.names=TRUE)
  ...
  # Create plot
  p <- ggplot2::ggplot(data=survival_table,
                       mapping=ggplot2::aes(x=!!sym("time"),
                                            y=!!sym("group_name"),
  ...
  p <- p + ggplot2::scale_y_discrete(breaks=rev(levels(survival_table$group_name)),
  ...
  # Set to p
  p$custom_grob <- list("heights"=list("name"="panel",
