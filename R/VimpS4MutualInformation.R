...
setMethod(
  ...
  function(object, data, ...) {
    ...
    if (length(signature_feature) > 0L) {
      # Iteration counter.
      ii <- 1L
      ...
      for (current_feature in signature_feature) {
        ...
        if (length(available_features) == 0L) break
        ...
        if (object@vimp_method == "mifs") {
          ...
          mi_data[
            available == TRUE,
            "objective_score" := mi - mi_redundancy / (ii - 1L)
          ]
      ...
      best_feature <- mi_data[objective_score == max_objective_score, ]$name[1L]
      mi_data[
        name %in% best_feature,
        ":="(
          "selected" = TRUE,
    ...
    while (length(available_features) > 0L) {
      ...
      if (!any(mi_data$available)) break
      
      # Select best feature and update mi_data.
      max_objective_score <- max(mi_data[available == TRUE]$objective_score)
      best_feature <- mi_data[available == TRUE & objective_score == max_objective_score, ]$name[1L]
      mi_data[
        name %in% best_feature,
        ":="(
          "selected" = TRUE, 
...
..compute_mutual_information_any_survival <- function(x, y) {
  if (is.factor(x)) {
    ...
    mi <- -0.5 * log(1.0 - (2.0 * (ci - 0.5))^2.0 + 1.0E-10)
...
..compute_mutual_information_continuous_discrete <- function(x, y) {
  ...
  mi <- apply(
    ...
    function(y, x) {
      return(
        -0.5 * log(1.0 - stats::cor(x = x, y = y, method = "spearman")^2.0 + 1.0E-10)
      )
    },
    x = x
  )
...
..compute_mutual_information_discrete_discrete <- function(x, y) {
  ...
  h_j <- -sum(p$prob_j$prob_j * log(p$prob_j$prob_j + 1.0E-10))
  h_k <- -sum(p$prob_k$prob_k * log(p$prob_k$prob_k + 1.0E-10))
  h_jk <- -sum(p$prob_kj$prob_kj * log(p$prob_kj$prob_kj + 1.0E-10))
...
..compute_mutual_information_continuous_continuous <- function(x, y) {
  ...
  return(-0.5 * log(1.0 - stats::cor(x = x, y = y, method = "spearman")^2.0 + 1.0E-10))
}
