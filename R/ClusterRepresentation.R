...
setMethod("add_feature_info_parameters", signature(object="clusteringObject", data="dataObject"),
          function(object,
            ...
            if(object@representation_method == "none"){
              ...
              representation_object_list <- .cluster_representation_by_best_predictor(object=object,
                                                                                      data=data)
...
.cluster_representation_by_medioid <- function(object,
  ...
  distance_table <- distance_table[element_1 %in% object@cluster_features & element_2 %in% object@cluster_features]
...
.cluster_representation_by_best_predictor <- function(object,
                                                      data,
                                                      ...){
  ...
  feature_p_values <- compute_univariable_p_values(data_obj=data,
                                                   feature_columns=object@cluster_features)
  
  # Integrate in a data.table
  predictor_data <- data.table::data.table("name"=names(feature_p_values),
  ...
  # Find the feature to be used as the representative feature.
  representative_feature <- predictor_data[p_value == min(p_value)]$name[1]
...
.cluster_representation_as_mean_feature <- function(object,
                                                    data,
                                                    feature_info_list,
                                                    ...){
  ...
  if(any(sapply(feature_info_list[object@cluster_features], function(x) (x@feature_type)) == "factor")){
  ...
  ..cluster_representation_as_mean_feature <- function(current_feature,
    ...
    representation_object <- methods::new("clusterRepresentationObject",
                                          ...
                                          "cluster_name" = paste0(cluster_features[1], "_meta_cluster"),
                                          "cluster_size" = length(cluster_features),
                                          "cluster_features" = cluster_features,
  ...
  # features.
  feature_correlation <- sapply(data@data[, mget(object@cluster_features)],
                                stats::cor,
                                y=data@data[[object@cluster_features[1]]],
                                use="na.or.complete",
                                method="spearman")
  
  # Iterate over features to form representation objects.
  representation_object_list <- mapply(..cluster_representation_as_mean_feature,
