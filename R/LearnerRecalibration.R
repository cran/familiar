learner.recalibrate_model <- function(object, data, time=NULL) {
  ...
  model_predictions <- .predict(object=object,
                                data=data,
  ...
  if(object@outcome_type %in% c("binomial", "multinomial")){
    ...
    for(ii in seq_along(class_probability_columns)){
      ...
      # Parse formula
      model_formula <- stats::reformulate(termlabels=current_class_probability_column,
      ...
      calibration_model <- tryCatch(stats::glm(model_formula,
    ...
    calibration_model <- tryCatch(survival::coxph(formula,
                                      data=model_predictions,
...
learner.apply_calibration <- function(object, predictions){
  ...
  if(object@outcome_type %in% c("binomial", "multinomial")){
    ...
    for(ii in seq_along(class_probability_columns)){
      ...
      # current column.
      predicted_probability <- stats::predict.glm(object=object@calibration_model[[class_levels[ii]]],
                                                  newdata=predictions,
                                                  type="response")
    ...
    max_prob_class <- factor(class_levels[predictions[, max.col(.SD), .SDcols=class_probability_columns]], levels=class_levels)
    predictions[, "predicted_class":=max_prob_class]

  } else if(object@outcome_type == "survival") {
    
    # Predict cox PH relative risk
    predicted_outcome_value  <- predict(object=object@calibration_model[[1]],
                                        newdata=predictions,
                                        type="risk")
