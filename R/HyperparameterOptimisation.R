...
setMethod(
  ...
  function(
    ...
    data <- methods::new(
      ...
      delay_loading = TRUE,
      perturb_level = tail(object@run_table, n = 1L)$perturb_level,
...
setMethod(
  ...
  function(
    ...
    convergence_stopping = 3L,
    no_decent_model_stopping = 4L,
    time_limit = NULL,
    verbose = TRUE,
    message_indent = 0L,
    ...
    ...
    logger_message(
      paste0(
        ...
        paste_s(metric), ifelse(length(metric) > 1L, " metrics", " metric"),
    ...
    while (optimisation_step < n_max_optimisation_steps && !skip_optimisation) {
      ...
      if (
        stop_list$convergence_counter_score >= convergence_stopping ||
...
.create_hyperparameter_optimisation_initial_objects <- function(
  ...
  if (is.null(learner)) {
    ...
    object <- promote_learner(object = methods::new(
      "familiarModel",
      outcome_type = settings$data$outcome_type,
