...
setMethod("create_instance_weights", signature(data="dataObject"),
          function(data, method, outcome_type=NULL, ...){
            
            if(is.null(outcome_type)) outcome_type <- data@outcome_type
            
            # Pass to data.table.
            return(create_instance_weights(data=data@data,
                                           method=method,
...
setMethod("create_instance_weights", signature(data="NULL"),
...
setMethod("create_instance_weights", signature(data="data.table"),
          function(data, outcome_type, method=NULL, normalisation="none", ...){
            
            # Check that an outcome-type is provided.
            if(is.null(outcome_type)) ..error_reached_unreachable_code("create_instance_weights: an outcome type should be provided.")
            ...
            if(method == "inverse_number_of_samples"){
              ...
              # Unknown method.
              ..error_reached_unreachable_code(paste0("create_instance_weights: unknown class weighting method. Found: ", method))
            }
            ...
            if(normalisation == "sum_one"){
              ...
              ..error_reached_unreachable_code(paste0("create_instance_weights: unknown normalisation method. Found: ", normalisation))
            }
...
..compute_effective_number_of_samples_beta <- function(x){
  ...
  if(x >= 0.0) ..error_reached_unreachable_code(paste0("..compute_effective_number_of_samples_beta: parameter should be negative. Found: ", x))
...
.get_default_sample_weighting_method <- function(outcome_type){
  ...
  # Set default method.
  default_method <- ifelse(outcome_type %in% c("binomial", "multinomial"),
                           "inverse_number_of_samples",
                           "none")
  
  # Set hyperparameter.
  return(.set_hyperparameter(default=default_method,
                             type="factor",
