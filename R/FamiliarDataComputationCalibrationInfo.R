...
# familiarDataElementCalibrationInfo object ------------------------------------

setClass(
  "familiarDataElementCalibrationInfo",
...
# extract_calibration_info (generic) -------------------------------------------
...
setMethod(
  ...
  function(
    ...
    logger_message(
      paste0("Extracting calibration information."),
    ...
    calibration_info <- extract_dispatcher(
      FUN = .extract_calibration_info,
      cl = NULL,
...
.extract_calibration_info <- function(
  ...
  if (object@outcome_type == "survival") {
    data_element@value_column <- setdiff(colnames(data_element@data), "time")
...
setMethod(
  ...
  signature(x = "familiarDataElementCalibrationInfo"),
    ...
    if (x[[1L]]@outcome_type %in% c("continuous")) {
      ...
      if (is.null(x[[1L]]@grouping_column)) {
        ...
        data <- data[
          ...
          by = x[[1L]]@grouping_column
        ]
      }
      
    } else if (x[[1L]]@outcome_type %in% c("survival")) {
      
      # Identify all unique time points.
      new_time <- sort(unique(data$time))
      
      # Find non-time grouping columns.
      grouping_column <- setdiff(
        colnames(data), c("time", x[[1L]]@value_column)
      )
      
      # Select only unique data points to prevent warnings during approximation.
      data <- unique(data)
      ...
      data <- data[
        ...
        by = c(x[[1L]]@grouping_column),
        .SDcols = c(x[[1L]]@value_column)
      ]
      
    } else {
      ..error_outcome_type_not_implemented(outcome_type = x[[1L]]@outcome_type)
    }
...
..interpolate_survival_data <- function(
    data,
    time,
    new_time,
    extrapolate = FALSE
  ...
  data <- data[!is.na(value_columns[1L])]
...
# export_calibration_info (general) --------------------------------------------
