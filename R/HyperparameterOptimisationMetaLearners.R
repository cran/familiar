.create_hyperparameter_challenger_sets <- function(
  ...
  # hyperparameters.
  n_max_local_steps <- 2L * sum(sapply(parameter_list, function(x) (x$randomise)))
  ...
  if (
    ...
    summary_score_table <- summary_score_table[, "n_select" := round(
      2.0 * n_local_sets * weight^2.0 / sum(weight^2.0)
    )][order(-summary_score)]
  ...
  if (is(score_optimisation_model, "familiarHyperparameterRandomSearch")) {
    ...
    if (!is_empty(random_sets)) {
      ...
      if (!all(random_sets$is_observed) && n_random_sets > 0L) {
      ...
      if (any(random_sets$is_observed) && n_random_sets > 0L) {
    ...
    if (!is_empty(local_sets)) {
      ...
      if (any(local_sets$is_observed) && n_local_sets > 0L) {
    ...
    if (!is_empty(random_sets) && !is_empty(local_challenger_sets) > 0L) {
      random_sets <- random_sets[
        !local_challenger_sets,
        on = .NATURAL
      ]
  ...
  if (!is_empty(challenger_sets)) {
    ...
    if (anyNA(challenger_sets$param_id)) {
...
..hyperparameter_random_forest_time_learner <- function(
  ...
  joint_table[is.na(time_taken), time_taken := 10.0 * max_time_taken]
...
acquisition_upper_confidence_bound <- function(
  ...
  beta <- 2.0 / 5.0 * log(5.0 / 3.0 * n_parameters * (acquisition_data$t + 1.0)^2.0 * pi^2.0)
...
acquisition_bayes_upper_confidence_bound <- function(
  ...
  q <- 1.0 - 1.0 / (acquisition_data$t + 1.0)
