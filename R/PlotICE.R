...
#'   the `value_scales` parameter. This parameter is ignored if
#'   `show_novelty=FALSE`.
...
#'
#' @description This method creates partial dependence plots based on data in a
  ...
  function(
    ...
    if (is.null(split_by) && is.null(facet_by) && is.null(color_by)) {
      if (show_2d) {
        ...
        color_by <- NULL
        if (object@outcome_type == "multinomial") {
          color_by <- c("positive_class")
        }
        if (object@outcome_type == "survival") {
          color_by <- c("evaluation_time")
        }
      }
    }
...
.plot_ice <- function(
  ...
  for (ii in names(layout_split)) {
    ...
    g_ice <- .rename_plot_grobs(
      g = .convert_to_grob(p_ice),
      extension = "main"
    )
    if (!gtable::is.gtable(g_ice)) next
