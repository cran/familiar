...
setGeneric("plot_auc_precision_recall_curve",
           function(object,
                    draw=FALSE,
                    dir_path=NULL,
                    split_by=NULL,
...
#'@rdname plot_auc_precision_recall_curve-methods
setMethod("plot_auc_precision_recall_curve", signature(object="ANY"),
          function(object,
                   draw=FALSE,
                   dir_path=NULL,
                   split_by=NULL,
            ...
            return(do.call(plot_auc_precision_recall_curve,
                           args=list("object"=object,
                                     "draw"=draw,
                                     "dir_path"=dir_path,
                                     "split_by"=split_by,
...
#'@rdname plot_auc_precision_recall_curve-methods
setMethod("plot_auc_precision_recall_curve", signature(object="familiarCollection"),
          function(object,
                   draw=FALSE,
                   dir_path=NULL,
                   split_by=NULL,
...
.plot_auc_curve <- function(object,
                            curve_type,
                            draw=FALSE,
                            dir_path=NULL,
                            split_by=NULL,
  ...
  if(is.list(x)){
    ...
    if(length(x) > 1) ..error_reached_unreachable_code(".plot_auc_curve: list of data elements contains unmerged elements.")
  ...
  .check_parameter_value_is_valid(x=curve_type,
                                  var_name="curve_type",
                                  values=c("roc", "pr"))
  ...
  if(is.null(split_by) & is.null(facet_by) & is.null(color_by)){
    ...
    n_positive_class <- ifelse(object@outcome_type == "binomial", 1L, nlevels(x@data$positive_class))
