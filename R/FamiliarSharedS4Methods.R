...
.familiar_version_string <- function(object) {
  ...
  if (length(object@familiar_version) > 1L) {
...
setMethod(
  ...
  function(
    ...
    if (object@outcome_type == "survival") {
      ...
      if (length(censoring_variable) > 0L) {
      ...
      # The same as above.
      if (all(data@data$outcome_event == 0L)) {
        return_value <- TRUE
        attr(return_value, "error") <- paste0(
          "All instances in the data set were censored. ",
          "Events are required for training the model."
      ...
      if (all(data@data$outcome_time == data@data$outcome_time[1L])) {
        return_value <- TRUE
        attr(return_value, "error") <- paste0(
      ...
      if (data.table::uniqueN(data@data$outcome) == 1L) {
        return_value <- TRUE
        attr(return_value, "error") <- paste0(
      ...
      if (all(data@data$outcome == data@data$outcome[1L])) {
        return_value <- TRUE
        attr(return_value, "error") <- paste0(
