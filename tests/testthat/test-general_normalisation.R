normalisation_test <- function(data_list,
                               norm_method,
                               expected_shift=NULL,
                               expected_scale=NULL,
                               expected_min,
  ...
  for(ii in seq_along(data_list)){
    ...
    # Create skeleton
    object <- familiar:::..create_normalisation_parameter_skeleton(feature_name="test",
                                                                   method=norm_method)
    ...
    if(expected_none[ii]){
      testthat::expect_equal(is(object, "featureInfoParametersNormalisationNone"), TRUE)
    ...
    # Apply normalisation
    y <- familiar:::apply_feature_info_parameters(object=object,
                                                  data=x)
  ...
  for(normalisation_method in familiar:::.get_available_normalisation_methods()){
    ...
    testthat::test_that(paste0("Normalisation is correctly performed using the ",
                               ...
                               " numeric features."),
                        {
                            ...
                            for(feature in familiar:::get_feature_columns(data_copy)){
                              ...
                              if(feature_info_list[[feature]]@feature_type == "numeric"){
                                ...
                                if(normalisation_method %in% c("normalisation", "normalisation_trim", "normalisation_winsor")){
                                  # Check that the feature is correctly centred
                                  # around 0.
                                ...
                                # For categorical features test that the object
                                # class is
                                # featureInfoParametersNormalisationNone.
                                testthat::expect_equal(is(feature_info_list[[feature]]@normalisation_parameters,
                                                          "featureInfoParametersNormalisationNone"),
                                                       TRUE)
                              }
                            }
                          ...
                          # Assert that inverting the transformation produces
                          # the original dataset.
                          data_restored <- familiar:::normalise_features(data=data_copy,
                                                                         feature_info_list=feature_info_list,
                                                                         invert=TRUE)
                          
                          # Iterate over features and compare. They should be
                          # equal.
                          ...
                          # Assert that aggregating the normalisation parameters
                          # works as expected.
                          ...
                          if(n_numeric_features > 0 & normalisation_method != "none"){
                            # Expect that the selected normalisation method
                            # matches the selected method.
                            testthat::expect_equal(aggr_normalisation_parameters$parameters@method, normalisation_method, ignore_attr=TRUE)
                            
                            # Expect that the shift and scale parameters are
                            # finite.
                            testthat::expect_equal(is.finite(aggr_normalisation_parameters$parameters@shift), TRUE)
                            if(is(aggr_normalisation_parameters$parameters, "featureInfoParametersNormalisationShiftScale")){
                              testthat::expect_equal(is.finite(aggr_normalisation_parameters$parameters@scale), TRUE)
                            }
                            
                            # Expect that instance mask is equal to the number
                            # of numeric features.
                            ...
                            for(feature in familiar:::get_feature_columns(data_copy)){
                              ...
                              if(feature_info_list[[feature]]@feature_type == "numeric"){
                                ...
                                if(feature == "feature_2"){
                                  # Assert that for the NA feature, the none
                                  # method is used.
                                  testthat::expect_equal(is(feature_info_list[[feature]]@normalisation_parameters,
                                                            "featureInfoParametersNormalisationNone"),
                                                         TRUE)
