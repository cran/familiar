...
experiment_data_assignment <- familiar::precompute_data_assignment(data=data,
...
experiment_feature_info <- familiar::precompute_feature_info(data=data,
...
testthat::test_that("Assert that feature info computation performs correctly", {
  ...
  for(feature_info_set in names(experiment_feature_info@feature_info)){
    for(feature_info in experiment_feature_info@feature_info[[feature_info_set]]){
      ...
      testthat::expect_equal(familiar:::feature_info_complete(feature_info), feature_info_set != "generic")
    }
  }
  
  # Test contents. 
  testthat::expect_equal(is.null(experiment_feature_info@vimp_table_list), TRUE)
  testthat::expect_equal(experiment_data_assignment@experiment_setup,
                         experiment_feature_info@experiment_setup,
                         ignore_attr=TRUE)
  testthat::expect_equal(experiment_data_assignment@iteration_list,
                         experiment_feature_info@iteration_list,
                         ignore_attr=TRUE)
  testthat::expect_equal(experiment_data_assignment@project_id,
...
experiment_vimp <- familiar::precompute_vimp(data=data,
...
testthat::test_that("Assert that variable importance computation performs correctly", {
  ...
  testthat::expect_equal(length(experiment_vimp@vimp_table_list$univariate_regression), 3L)
  ...
  testthat::expect_equal(experiment_data_assignment@iteration_list,
                         ...
                         ignore_attr=TRUE)
  testthat::expect_equal(experiment_feature_info@feature_info,
  ...
  vimp_table <- familiar::aggregate_vimp_table(experiment_vimp, aggregation_method="borda")
  testthat::expect_s4_class(vimp_table$univariate_regression, "vimpTable")
  vimp_table <- familiar::get_vimp_table(vimp_table)
  testthat::expect_s3_class(vimp_table$univariate_regression, "data.table")
})


# Add additional variable importance methods.
experiment_vimp_3 <- familiar::precompute_vimp(data=data,
...
testthat::test_that("Addition variable importance computation performs correctly", {
  ...
  for(ii in seq_along(experiment_vimp_3@vimp_table_list$univariate_regression)){
    testthat::expect_equal(experiment_vimp_3@vimp_table_list$univariate_regression[[ii]],
                           experiment_vimp@vimp_table_list$univariate_regression[[ii]],
                           ignore_attr=TRUE)
  }
  
  # Test that 9 variable importance tables were created.
  testthat::expect_equal(length(unlist(familiar::get_vimp_table(experiment_vimp_3), recursive=FALSE)),
                         9L)
  
  # Test that 3 aggregated variable importance tables are created.
  testthat::expect_equal(length(familiar::get_vimp_table(familiar::aggregate_vimp_table(experiment_vimp_3, "borda"))),
                         3L)
  
  testthat::expect_equal(experiment_vimp@project_id,
