# Create prototype.
proto_vimp_table <- familiar:::prepare_vimp_table_object(vimp_method="mim",
                                                         invert=TRUE,
                                                         state="declustered")

# Fill a list with multiple prototypes.
vimp_table_list <- list(proto_vimp_table,
                        proto_vimp_table,
                        proto_vimp_table)

# Update with actual data.
vimp_table_list[[1]]@vimp_table <- data.table::data.table("score"=c(1.0, 0.8, 0.6, 0.4, 0.2),
                                                          "name"=c("a", "b", "c", "d", "e"))
vimp_table_list[[2]]@vimp_table <- data.table::data.table("score"=c(0.8, 1.0, 0.6),
                                                          "name"=c("a", "b", "c"))
vimp_table_list[[3]]@vimp_table <- data.table::data.table("score"=c(0.6, 1.0),
                                                           "name"=c("a", "e"))
...
testthat::test_that("Mean rank aggregation is correct", {
  ...
  aggregated_rank_table <- familiar:::aggregate_vimp_table(vimp_table_list,
                                                           aggregation_method="mean")

  testthat::expect_equal(familiar:::get_vimp_table(aggregated_rank_table),
                         expected_rank_table,
                         ignore_attr=TRUE)
})
